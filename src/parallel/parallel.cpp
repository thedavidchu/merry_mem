...
ParallelRobinHoodHashTable::insert(KeyType key, ValueType value)
{
    ...
    auto [next_index, found] = this->find_next_index_lock(manager, home, key, offset);
...
ParallelRobinHoodHashTable::insert_or_update(KeyType key, ValueType value)
{
    ...
    for(size_t distance = 0; this->buckets_[home].offset >= distance; ++distance, ++idx){
        while(this->buckets_[idx].key == key) {
            ...
            auto [prev_key, prev_val] = this->compare_and_set_key_val(home, prev_key_copy, this->buckets_[idx].key, prev_val_copy);
            if (prev_key == key) {
                return;
            }
        }
    }
    auto [is_inserted, is_found] = this->distance_zero_insert(key, value, home);
    if(is_inserted) {
        return;
    }
    ...
    this->locked_insert(entry_to_insert, next_index);
    manager.release_all_locks();
    return;
}
